...
function llm_gpt_dygraph_auto_bs8_fp32_DP2() {
    ...
    loss_base=10.52195549 # output of dropout is different after supporting spmd
    ips_base=-1
    mem_base=-1
    if [ $IS_A100 -ne 0 ];then
        loss_base=10.51487541 # after add dropout spmd
    fi
    check_result $FUNCNAME ${loss_base} ${loss} ${ips_base} ${ips} ${mem_base} ${mem}
    echo "=========== $FUNCNAME run  end ==========="
}
...
function llm_gpt_dygraph_auto_bs8_fp32_DP2-MP2() {
    ...
    mem=-1
    echo "result: loss=$loss ips=$ips mem=$mem loss_md5=$loss_md5"
    loss_base=10.53319931 # output of dropout is different after supporting spmd
    ips_base=-1
    mem_base=-1
    if [ $IS_A100 -ne 0 ];then
        loss_base=10.53310299 # after add dropout spmd
    fi
    check_result $FUNCNAME ${loss_base} ${loss} ${ips_base} ${ips} ${mem_base} ${mem}
    echo "=========== $FUNCNAME run  end ==========="
}
...
function llm_gpt_dygraph_auto_bs8_fp32_DP2-MP2-PP2() {
    ...
    loss_base=10.53350258 # output of dropout is different after supporting spmd
    ips_base=-1
    mem_base=-1
    if [ $IS_A100 -ne 0 ];then
        loss_base=10.53452206 # after add dropout spmd
    fi
    check_result $FUNCNAME ${loss_base} ${loss} ${ips_base} ${ips} ${mem_base} ${mem}
    echo "=========== $FUNCNAME run  end ==========="
}
...
function llm_gpt_dygraph_auto_bs8_fp16_DP2-MP2-PP2() {
    ...
    loss_base=10.53543472
    ips_base=-1
    mem_base=-1
    if [ $IS_A100 -ne 0 ];then
        loss_base=10.53643703 # after add dropout spmd
    fi
    check_result $FUNCNAME ${loss_base} ${loss} ${ips_base} ${ips} ${mem_base} ${mem}
    echo "=========== $FUNCNAME run  end ==========="
}
...
function llm_gpt_dygraph_auto_bs8_fp16_DP2-MP2-PP2_intermediate() {
    ...
    echo "result: loss=$loss ips=$ips mem=$mem loss_md5=$loss_md5"
    # loss_base=10.58456802     # note: need to debug
    loss_base=10.52467251
    ips_base=-1
    mem_base=-1
    if [ $IS_A100 -ne 0 ];then
        loss_base=10.52024651 # after add dropout spmd
    fi
    check_result $FUNCNAME ${loss_base} ${loss} ${ips_base} ${ips} ${mem_base} ${mem}
    echo "=========== $FUNCNAME run  end ==========="
}
