...
def main():
    ...
    def forward_post_hook(layer, input, output):
        ...
        if hasattr(layer, "weight") and layer.weight is not None:
            print(
                f"local weight dtype : {layer.weight._local_value().dtype} shape : {layer.weight._local_value().shape}  _md5sum : {layer.weight._local_value()._md5sum()[:5]}"
            )
        if hasattr(layer, "bias") and layer.bias is not None:
            print(
                f"local bias dtype : {layer.bias._local_value().dtype} shape : {layer.bias._local_value().shape} _md5sum : {layer.bias._local_value()._md5sum()[:5]}"
            )
        for inp in input:
            if inp is not None and hasattr(inp, "_md5sum"):
                print(
                    f"input local_dtype : {inp._local_value().dtype} local_shape : {inp._local_value().shape} local_md5 : {inp._local_value()._md5sum()[:5]} all_md5 {inp._md5sum()[:5]}"
                )
        for out in output:
            if out is not None and hasattr(out, "_md5sum"):
                print(
                    f"output local_dtype : {out._local_value().dtype} local_sahpe : {out._local_value().shape} local_md5 : {out._local_value()._md5sum()[:5]} all_md5 {out._md5sum()[:5]}"
                )
